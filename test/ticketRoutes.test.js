...
const routes_1 = __importDefault(require("../src/api/v1/routes"));
...
describe("Ticket Routes", () => {
    ...
    describe("POST /api/v1/tickets", () => {
        it("should create a ticket with 201", async () => {
            const res = await (0, supertest_1.default)(app).post("/api/v1/tickets").send({
                title: "Test ticket",
    ...
    describe("GET /api/v1/tickets/:id", () => {
        it("should return 404 if ticket not found", async () => {
            const res = await (0, supertest_1.default)(app).get("/api/v1/tickets/999999");
    ...
    describe("PUT /api/v1/tickets/:id", () => {
        it("should return 400 for invalid priority", async () => {
            const created = await (0, supertest_1.default)(app).post("/api/v1/tickets").send({
                title: "Update priority test",
                description: "desc",
                priority: "low"
            });
            const id = created.body.id;
            const res = await (0, supertest_1.default)(app).put(`/api/v1/tickets/${id}`).send({
                priority: "urgent"
            });
            expect(res.status).toBe(400);
            expect(res.body.message).toBe("Invalid priority. Must be one of: critical, high, medium, low");
        });
    });
    describe("DELETE /api/v1/tickets/:id", () => {
        it("should delete ticket and return 200", async () => {
            ...
            const id = created.body.id;
            const res = await (0, supertest_1.default)(app).delete(`/api/v1/tickets/${id}`);
            expect(res.status).toBe(200);
    ...
    describe("GET /api/v1/tickets/:id/urgency", () => {
        it("should return urgency fields", async () => {
            ...
            const id = created.body.id;
            const res = await (0, supertest_1.default)(app).get(`/api/v1/tickets/${id}/urgency`);
            expect(res.status).toBe(200);
